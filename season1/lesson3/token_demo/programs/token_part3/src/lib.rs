...
declare_id!("9Py1ZLhBLSw2UYJqB2EQMAnNC8z3RWaNfB4Z2Kh2UCrn");

#[program]
pub mod token_part3 {
    use super::*;

    /// PART3: 创建带 Metaplex 元数据的代币
    pub fn create_token(ctx: Context<CreateToken>) -> Result<()> {
        msg!("=== PART3: 创建带 Metaplex 元数据的代币 ===");
        msg!("代币 Mint 地址: {:?}", ctx.accounts.mint.key());
        msg!("元数据账户: {:?}", ctx.accounts.metadata_account.key());
        msg!("代币创建者: {:?}", ctx.accounts.authority.key());
        msg!("使用 TOKEN_PROGRAM_ID + METADATA_PROGRAM_ID 创建代币");
        msg!("代币创建成功！");
        Ok(())
    }
