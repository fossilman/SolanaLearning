...
import { TokenPart1 } from "../target/types/token_part1";
import {
  TOKEN_PROGRAM_ID,
...
describe("token_part1", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.tokenPart1 as Program<TokenPart1>;

  it("PART1: 演示代币生命周期（TOKEN_PROGRAM_ID，不包含元数据）", async () => {
    console.log("\n========== PART1: 代币生命周期演示开始 ==========\n");
    ...
    // 创建代币 Mint
    console.log("2. 创建代币 Mint（不包含元数据）");
    const mintKeypair = Keypair.generate();
    const mint = mintKeypair.publicKey;
    console.log("   - Mint 地址:", mint.toString());
    console.log("   - 使用 TOKEN_PROGRAM_ID 创建代币");
    ...
    // 总结
    console.log("========== PART1: 代币生命周期演示完成 ==========");
    console.log("\n总结:");
    console.log("  1. ✓ 创建代币 Mint（不包含元数据）");
