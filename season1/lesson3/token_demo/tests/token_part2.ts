...
import { TokenPart2 } from "../target/types/token_part2";
import {
...
describe("token_part2", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.tokenPart2 as Program<TokenPart2>;

  it("PART2: 演示 Token-2022 代币生命周期（包含元数据）", async () => {
    console.log("\n========== PART2: Token-2022 代币生命周期演示开始 ==========\n");
    ...
    console.log("   - Token-2022 程序 ID:", TOKEN_2022_PROGRAM_ID.toString());
    console.log("");

    // 创建 Token-2022 代币 Mint
    console.log("2. 创建 Token-2022 代币 Mint（包含元数据）");
    const mintKeypair = Keypair.generate();
    const mint = mintKeypair.publicKey;
    console.log("   - Mint 地址:", mint.toString());
    console.log("   - 使用 TOKEN_2022_PROGRAM_ID 创建代币");
    
    // 打印创建前的账户状态
    console.log("\n   [创建前账户状态]");
    await printAccountInfo(connection, mint, "Mint账户（创建前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印创建后的账户状态
    console.log("   [创建后账户状态]");
    await printAccountInfo(connection, mint, "Mint账户（创建后）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, payer.publicKey, "支付账户（Authority）");
    
    console.log("   ✓ Token-2022 代币创建成功\n");

    // 创建代币账户（发送方）
    console.log("3. 创建发送方的 Token-2022 代币账户");
    ...
    // 打印创建前的账户状态
    console.log("\n   [创建前账户状态]");
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，创建前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印创建后的账户状态
    console.log("   [创建后账户状态]");
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，创建后）", TOKEN_2022_PROGRAM_ID);
    ...
    console.log("5. 创建接收方的 Token-2022 代币账户");
    ...
    // 打印创建前的账户状态
    console.log("\n   [创建前账户状态]");
    await printAccountInfo(connection, receiverTokenAccount, "Token账户（接收方，创建前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印创建后的账户状态
    console.log("   [创建后账户状态]");
    await printAccountInfo(connection, receiverTokenAccount, "Token账户（接收方，创建后）", TOKEN_2022_PROGRAM_ID);
    
    console.log("   ✓ 接收方代币账户创建成功\n");

    // 铸造代币
    console.log("6. 铸造 Token-2022 代币到发送方账户");
    const mintAmount = 1000 * 10 ** 9; // 1000 个代币（考虑 9 位小数）
    console.log("   - 铸造数量: 1000 代币");
    
    // 打印铸造前的账户状态
    console.log("\n   [铸造前账户状态]");
    await printAccountInfo(connection, mint, "Mint账户（铸造前）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，铸造前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印铸造后的账户状态
    console.log("   [铸造后账户状态]");
    await printAccountInfo(connection, mint, "Mint账户（铸造后）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，铸造后）", TOKEN_2022_PROGRAM_ID);
    
    console.log("   ✓ Token-2022 代币铸造成功\n");

    // 转账代币
    console.log("7. 从发送方转账 Token-2022 代币到接收方");
    const transferAmount = 200 * 10 ** 9; // 200 个代币
    console.log("   - 转账数量: 200 代币");
    
    // 打印转账前的账户状态
    console.log("\n   [转账前账户状态]");
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，转账前）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, receiverTokenAccount, "Token账户（接收方，转账前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印转账后的账户状态
    console.log("   [转账后账户状态]");
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，转账后）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, receiverTokenAccount, "Token账户（接收方，转账后）", TOKEN_2022_PROGRAM_ID);

    console.log("   ✓ Token-2022 代币转账成功\n");

    // 销毁代币
    console.log("8. 销毁发送方账户中的 Token-2022 代币");
    const burnAmount = 100 * 10 ** 9; // 100 个代币
    console.log("   - 销毁数量: 100 代币");
    
    // 打印销毁前的账户状态
    console.log("\n   [销毁前账户状态]");
    await printAccountInfo(connection, mint, "Mint账户（销毁前）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，销毁前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印销毁后的账户状态
    console.log("   [销毁后账户状态]");
    await printAccountInfo(connection, mint, "Mint账户（销毁后）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, senderTokenAccount, "Token账户（发送方，销毁后）", TOKEN_2022_PROGRAM_ID);

    console.log("   ✓ Token-2022 代币销毁成功\n");

    // 关闭代币账户
    console.log("9. 关闭接收方的 Token-2022 代币账户");
    console.log("   - 注意: 关闭账户会将账户中的剩余代币销毁，并将租金返还给接收方");
    
    // 打印关闭前的账户状态
    console.log("\n   [关闭前账户状态]");
    await printAccountInfo(connection, receiverTokenAccount, "Token账户（接收方，关闭前）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, receiver.publicKey, "接收方系统账户（关闭前）");
    await printAccountInfo(connection, mint, "Mint账户（关闭前）", TOKEN_2022_PROGRAM_ID);
    ...
    // 打印关闭后的账户状态
    console.log("   [关闭后账户状态]");
    await printAccountInfo(connection, receiverTokenAccount, "Token账户（接收方，关闭后）", TOKEN_2022_PROGRAM_ID);
    await printAccountInfo(connection, receiver.publicKey, "接收方系统账户（关闭后）");
    await printAccountInfo(connection, mint, "Mint账户（关闭后）", TOKEN_2022_PROGRAM_ID);
    
    console.log("   ✓ Token-2022 代币账户关闭成功\n");

    // 总结
    console.log("========== PART2: Token-2022 代币生命周期演示完成 ==========");
    console.log("\n总结:");
    console.log("  1. ✓ 创建 Token-2022 代币 Mint（包含元数据）");
